...
from tinygrad.sunnypilot.modeld_v2.helpers import all_same, colored, getenv, DEBUG, GlobalCounters, ansilen, BEAM, NOOPT, all_int, CAPTURING, Metadata, TRACEMETA
from tinygrad.sunnypilot.modeld_v2.ops import Ops, PatternMatcher, UOp, UPat, Variable, sym_infer
from tinygrad.sunnypilot.modeld_v2.device import Device, Buffer
from tinygrad.sunnypilot.modeld_v2.renderer import Renderer, ProgramSpec, Estimates
from tinygrad.sunnypilot.modeld_v2.codegen.kernel import Kernel
from tinygrad.sunnypilot.modeld_v2.engine.schedule import ScheduleItem
...
def get_kernel(renderer:Renderer, ast:UOp) -> Kernel:
  ...
  if not NOOPT:
    ...
    if BEAM >= 1:
      from tinygrad.sunnypilot.modeld_v2.engine.search import beam_search, bufs_from_lin
...
class CompiledRunner(Runner):
  ...
  def __call__(self, rawbufs:list[Buffer], var_vals:dict[Variable, int], wait=False) -> Optional[float]:
    ...
    if global_size is not None and local_size is None and all_int(self.p.global_size): # type: ignore[arg-type]
      ...
      from tinygrad.sunnypilot.modeld_v2.engine.search import optimize_local_size
